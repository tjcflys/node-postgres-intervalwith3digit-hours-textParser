...
var MON = NUM + '\\s+mons?';
var DAY = NUM + '\\s+days?';
var TIME1 = '([+-])?(\\d\\d\\d):(\\d\\d):(\\d\\d):?(\\d\\d\\d)?';
var TIME2 = '([+-])?(\\d\\d):(\\d\\d):(\\d\\d):?(\\d\\d\\d)?';
var INTERVAL1 = [YEAR,MON,DAY,TIME1].map(function(p){
  return "("+p+")?";
}).join('\\s*');
var INTERVAL2 = [YEAR,MON,DAY,TIME2].map(function(p){
  return "("+p+")?";
}).join('\\s*');
...
var parseInterval = function(val) {
  ...
  if(valsplit[0][2]){
	var m = new RegExp(INTERVAL1).exec(val);  
  }else{
	  var m = new RegExp(INTERVAL2).exec(val);
  }
